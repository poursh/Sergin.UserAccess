...
5. Presentation: `Sergin.UserAccess.Presentation.WebApi/<Aggregate>/Endpoints/<Feature>/<Feature>Endpoint.cs` implementing `IEndpoint.MapEndpoint`, mapping the appropriate HTTP verb, binding a request model (add one alongside the endpoint if the command needs a body, e.g. `New<X>Model.cs`), sending via `ISerginSender` (`Sergin.SharedKernel.Application.Dispatching`) — `ErrorOr<TResponse> res = await sender.SendAsync(new <Feature>Command(...));` — returning `res.ToApiResult()`.
