...
- **Inject `ISerginUiDispatcher`, never `ISender` or `IMediator`.** In Blazor Server, "scoped" means the SignalR circuit's lifetime (hours), not a single HTTP request. Resolving `ISender` off the circuit scope would share one `DbContext` across every user interaction, creating an unbounded change tracker, stale reads, and "a second operation was started on this context" errors on concurrent renders. The dispatcher opens a fresh DI scope per send, isolating each operation's `DbContext`. Inject it as a parameter in the component's primary constructor or via `[Inject] ISerginUiDispatcher Dispatcher { get; set; }` in the code-behind.
- **List pages use `MudTable` with `ServerData` and paging only** — no sort, filter, or search controls, because the shared `ListQuery<T>` drops `Filtering`/`Sorting` and no `IUserAllQueryRepository` query reads a `Term`. Surfacing UI controls that silently do nothing is worse than omitting them. Note that `MudTable`'s `TableState.Page` is 0-based while Sergin's `ListQuery.PageIndex` is 1-based — adjust when building the query.
