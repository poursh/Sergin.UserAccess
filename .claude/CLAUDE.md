...
`Sergin.UserAccess.Domain/Roles/Role.cs` — `AggregateRoot<RoleId>`, private ctor + `static Create(RoleName, IEnumerable<PermissionCode>)`, with `Grant`/`Revoke` mutators. `RoleId`, `RoleName` and `PermissionCode` are trailing records in the same file.

**`PermissionCode` is deliberately not `Sergin.SharedKernel.Application.Securities.Permission`.** That type lives in the Application layer and a `.Domain` project references only `SharedKernel.Domain`, so the domain stores the raw string and the Application layer validates it through `Permission.Create`/`TryCreate` on the way in and out. Don't "fix" this by moving `Permission` down into `SharedKernel.Domain` — that is a breaking rename across all three repos.
