...
- **Inject `ISerginSender`, never MediatR's `ISender`/`IMediator` directly.** `ISerginSender` (`Sergin.SharedKernel.Application.Dispatching`) is no longer Blazor-specific — this module's WebApi endpoints inject it too (see `CreateUserEndpoint`), so the type is the shared dispatch entry point across both presentation layers, not a UI-only wrapper. In Blazor Server specifically, "scoped" means the SignalR circuit's lifetime (hours), not a single HTTP request. Resolving MediatR's `ISender` off the circuit scope would share one `DbContext` across every user interaction, creating an unbounded change tracker, stale reads, and "a second operation was started on this context" errors on concurrent renders. `ISerginSender.SendAsync(...)` opens a fresh DI scope per send, isolating each operation's `DbContext`. Inject it as a parameter in the component's primary constructor or via `[Inject] ISerginSender Sender { get; set; }` in the code-behind.
